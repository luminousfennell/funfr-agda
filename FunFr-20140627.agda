...
module STLC where
  ...
  module Try2 where
    ...
    data Bound : Nat -> Ctx -> Set where
      hd : {G : Ctx} {t : Ty} -> Bound zero ( t :: G)
      tl : {n : Nat} {t : Ty} {G : Ctx} -> Bound n G -> Bound (suc n) (t :: G)
      
    lookup : {n : Nat} (G : Ctx) -> Bound n G -> Ty
    lookup nil ()
    lookup (t :: G) hd = t
    lookup (_ :: G) (tl x) = lookup G x
    ...
    data Exp : Ctx -> Ty -> Set where
      ...
      Lam : {G : Ctx} -> (t1 : Ty) {t2 : Ty} -> Exp (t1 :: G) t2 -> Exp G (Fun t1 t2)
      App : {G : Ctx} {t1 t2 : Ty} -> Exp G (Fun t1 t2) -> Exp G t1 -> Exp G t2
      Var : {G : Ctx} {n : Nat} (x : Bound n G) -> Exp G (lookup G x)

    -- we need to guarantee that variables are well-scoped; otherwise
    -- no chance for type safety:
    -- data Bound : Ty -> Ctx -> Set where
    --   hd : {t : Ty} {G : Ctx} -> Bound t (t :: G)
    --   tl : {t t' : Ty} {G : Ctx} -> Bound t G -> Bound t (t' :: G)


    -- data Exp : Ctx -> Ty -> Set where
    --   C : {G : Ctx} -> Nat -> Exp G N
    --   Add : {G : Ctx} -> Exp G N -> Exp G N -> Exp G N
    --   Lam : {G : Ctx} -> (t1 : Ty) {t2 : Ty} -> Exp (t1 :: G) t2 -> Exp (Fun t1 t2)
    --   App : {G : Ctx} {t1 t2 : Ty} -> Exp G (Fun t1 t2) -> Exp G t1 -> Exp G t2
    --   Var : {G : Ctx} -> Nat -> Exp G ...
